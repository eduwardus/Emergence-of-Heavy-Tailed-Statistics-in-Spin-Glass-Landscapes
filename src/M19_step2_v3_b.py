# -*- coding: utf-8 -*-
"""M19_step2_v3_b.py - VERSIÓN CORRECTA: d_G como DIMENSIÓN EFECTIVA (PCA entropy)"""
...
BASE_DRIVE = "/content/drive/MyDrive/M19_V3"  # Nueva versión
dirs = [
    BASE_DRIVE,
    f"{BASE_DRIVE}/data",
...
# =========================================================
# 1. COPIAR INSTANCIAS ORIGINALES
# =========================================================

# Buscar instancias originales en M19 o M19_V2
SOURCE_INSTANCES = None
for source in ["/content/drive/MyDrive/M19_V2/data/instances",
               "/content/drive/MyDrive/M19/data/instances"]:
    if os.path.exists(source):
        SOURCE_INSTANCES = source
        break

if SOURCE_INSTANCES is None:
    print("\n❌ ERROR: No se encontraron instancias originales")
    exit(1)

DRIVE_INSTANCES = f"{BASE_DRIVE}/data/instances"
print(f"\n📦 Copiando instancias desde {SOURCE_INSTANCES}...")
for f in Path(SOURCE_INSTANCES).glob("*.npz"):
    shutil.copy2(f, DRIVE_INSTANCES)
print(f"   ✅ Copiadas {len(list(Path(DRIVE_INSTANCES).glob('*.npz')))} instancias")
...
# =========================================================
# 5. d_G como DIMENSIÓN EFECTIVA (PCA + entropía)
# =========================================================
...
def compute_dG_effective(solutions):
    ...
    try:
        # Convertir soluciones a matriz (n_solutions × N)
...
class TimeoutExplorer:
    ...
    def get_results(self):
        ...
        return {
            'dG_effective': dG_effective,      # <--- NUEVA MÉTRICA (COMPARABLE CON M18)
...
print(f"⏱️  Timeout: {TIMEOUT_SECONDS//60} minutos")
...
for idx, (bin_name, npz_path) in enumerate(remaining):
    ...
    try:
        ...
        if is_partial:
            print(f"   ⏰ TIMEOUT ({elapsed:.1f}s) - PARCIAL ({results['completed_starts']}/{NUM_STARTS})")
...
print("="*70)
print("✅ M19_step2 V3 COMPLETADO")
print("   d_G ahora es DIMENSIÓN EFECTIVA (PCA + entropía)")
print("   COMPARABLE CON M18")
print("\n👉 Ejecuta M19_step3 con 'dG_effective' como observable")
